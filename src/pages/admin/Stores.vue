...
import type { Store, User } from '@/utils/types';
...
const readMachine = useRead<Store[], true>('/guest/stores', true, {
  route,
  router,
  extra
});
...
<template>
  <div class="page-container">
    <div class="block md:flex justify-between items-center">
      <h2 class="my-4">All Stores List</h2>
      ...
      >
        ...
        <tbody>
          <tr v-for="item in readMachine.response.value" :key="item.id">
            ...
            <td>
              <a
                :href="`https://${item.subDomain}.khudroshop.com`"
                target="_blank"
                class="text-blue-500 hover:underline"
              >
                {{ item.subDomain }}.khudroshop.com
              </a>
              <div v-if="item.customDomain">
                <a
                  :href="`https://${item.customDomain}`"
                  target="_blank"
                  class="text-blue-500 hover:underline"
                >
                  {{ item.customDomain }}
            ...
            <td>
              {{ item.thisMonthOrdersCount }} / {{ item.package?.monthlyOrdersLimit }} __ ({{
