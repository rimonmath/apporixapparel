...
const rootPages = import.meta.glob('../pages/customer/**/*.vue');
...
Object.keys(rootPages).forEach((filePath) => {
  const match = filePath.match(/\/customer\/(.+)\.vue$/);
  ...
  const route: TempRoute = {
    ...
    name: 'customer-' + relativePath.replace(/\//g, '_'),
  ...
  if (parentPath) {
    ...
    if (!routeMap.has(parentPath)) {
      routeMap.set(parentPath, {
        ...
        name: 'customer-' + parentPath.replace(/\//g, '_'),
        component: null,
        children: []
      });
    }
