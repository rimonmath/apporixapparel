...
const rootPages = import.meta.glob('../pages/store/**/*.vue');
...
Object.keys(rootPages).forEach((filePath) => {
  const match = filePath.match(/\/store\/(.+)\.vue$/);
  ...
  const route: TempRoute = {
    ...
    name: 'store-' + relativePath.replace(/\//g, '_'),
  ...
  if (parentPath) {
    ...
    if (!routeMap.has(parentPath)) {
      routeMap.set(parentPath, {
        ...
        name: 'store-' + parentPath.replace(/\//g, '_'),
        component: null,
        children: []
      });
    }
